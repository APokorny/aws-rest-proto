...
using request_token = detail::client_request;

template <typename RequestType, typename... Ts>
std::unique_ptr<request_token> async_request(boost::asio::ssl::stream<boost::asio::ip::tcp::socket>& stream, std::string const& region,
                                             Ts&&... ts) {
