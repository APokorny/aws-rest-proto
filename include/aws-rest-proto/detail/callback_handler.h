...
typename std::enable_if<has_key<ErrorName, Parameters>::value, bool>::type try_execute_error(Parameters const&         paras,
                                                                                             ...
                                                                                             boost::string_view const& error_msg,
                                                                                             nlohmann::json const&     payload)
{
    if (error_str == c_str(ErrorName{}))
    {
        get<ErrorName>(paras)(error_msg, payload);
        return false;
    }
    return true;
}
...
void for_each_error(t::error_response<Errors...>, Parameters const& paras, boost::string_view const& error_str,
                    boost::string_view const& error_msg, nlohmann::json const& payload)
{
    bool cont                        = true;
    bool execute_until_first_match[] = {cont = cont && try_execute_error<Errors>(paras, error_str, error_msg, payload)...};
    if (cont) get<t::any_other>(paras)(error_msg, payload);
}
