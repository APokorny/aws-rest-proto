#include "aws-rest-proto/client_request.h"
#include "encode.h"
...
void ad::client_request::write_and_read(std::function<void(beast::error_code)>&& fail_handler,
    ...
    http::async_write(stream, req, std::bind(&client_request::on_write, this, std::placeholders::_1, std::placeholders::_2));
}

void ad::client_request::on_write(boost::beast::error_code ec, std::size_t transfered) {
    if (ec) return error_handler(ec);
    http::async_read(stream, buffer, res, std::bind(&client_request::on_read, this, std::placeholders::_1, std::placeholders::_2));
}
